...
class HighlightView extends View {
  ...
  public static final int MOVE             = (1 << 5);
  	...
  	public HighlightView(Context context, View ctx) {
  		super(context);
  		mContext = ctx;
  	}
  	private void init() {
     ...
     public boolean onTouchEvent(MotionEvent event) {
    	 ...
    	 switch (event.getAction()) {
    	 	case MotionEvent.ACTION_DOWN: 
    	 		int edge = this.getHit(event.getX(), event.getY());
    	 		if (edge != this.GROW_NONE) {
    	 			this.setMode(
    	 	...
    	 	case MotionEvent.ACTION_MOVE:
    	 		handleMotion(getHit(event.getX(), event.getY()), 
    	 				event.getX() - mLastX, 
    	 				event.getY() - mLastY);
    	 		mLastX = event.getX();
    	 		mLastY = event.getY();
